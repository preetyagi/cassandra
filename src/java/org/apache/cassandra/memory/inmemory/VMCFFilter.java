...
public class VMCFFilter implements MCFFilter
{
    TableMetadata metadata;
    private Row.Builder builder;

    public VMCFFilter(TableMetadata metadata)
    {
    ...
    public Unfiltered getUnfilteredRowIterator(Map.Entry<MClusteringKey, MRow> persistentRowEntry, TableMetadata metadata)
    {
        ...
        ConcurrentNavigableMap<String, VMCellBase> cells = persistentRow.getCells();
        for (Map.Entry<String, VMCellBase> columnData : cells.entrySet())
        {
            ColumnMetadata column = metadata.getColumn(ByteBuffer.wrap(columnData.getKey().getBytes()));
            assert column != null : "column is null";
            if (columnData.getValue().getType() == MCellType.getType(MConfig.CELL_TYPE_SIMPLE))
            {

                MSimpleCell pcell = (MSimpleCell) columnData.getValue();
                Cell cassCell;
                if (pcell.getValue() != null)
                    cassCell = new BufferCell(column, pcell.getTimestamp(), pcell.getTtl(), pcell.getLocalDeletionTime(), ByteBuffer.wrap(pcell.getValue()), null);
                else
                    cassCell = new BufferCell(column, pcell.getTimestamp(), pcell.getTtl(), pcell.getLocalDeletionTime(), ByteBufferUtil.EMPTY_BYTE_BUFFER, null);
                builder.addCell(cassCell);
            }
            else if (columnData.getValue().getType() == MCellType.getType(MConfig.CELL_TYPE_COMPLEX))
            {

                MComplexCell persistentComplexCell = (MComplexCell) columnData.getValue();
                for (VMCell pcell : (List<VMCell>) persistentComplexCell.getCells())
                {

                    Cell cassCell;
                    if (pcell.getValue() != null)
                        cassCell = new BufferCell(column, pcell.getTimestamp(), pcell.getTtl(), pcell.getLocalDeletionTime(), ByteBuffer.wrap(pcell.getValue()),
                                                  CellPath.create(ByteBuffer.wrap(pcell.getCellPath())));
                    else
                        cassCell = new BufferCell(column, pcell.getTimestamp(), pcell.getTtl(), pcell.getLocalDeletionTime(), ByteBufferUtil.EMPTY_BYTE_BUFFER,
                                                  CellPath.create(ByteBuffer.wrap(pcell.getCellPath())));
    ...
    public Unfiltered getUnfilteredRowSingle(MRow mRow, Clustering clustering, TableMetadata metadata)
    {
        ...
        ConcurrentNavigableMap<String, VMCellBase> cells = mRow.getCells();
        for (Map.Entry<String, VMCellBase> columnData : cells.entrySet())
        {
            ColumnMetadata column = metadata.getColumn(ByteBuffer.wrap(columnData.getKey().getBytes()));
            assert column != null : "column metadata is null.";
            if (columnData.getValue().getType() == VMCellType.getCellType(MConfig.CELL_TYPE_SIMPLE))
            {
            ...
            else if (columnData.getValue().getType() == VMCellType.getCellType(MConfig.CELL_TYPE_COMPLEX))
            {

                VMComplexCell persistentComplexCell = (VMComplexCell) columnData.getValue();
                                                 ...
                                                 EncodingStats.NO_STATS)
        {
            protected Unfiltered computeNext()
            {
                while (clusteringIterator.hasNext())
                {
                    ...
                    pclKey.addClusteringKeyColumn("table_name", MDataTypes.getType(MConfig.PM_UTF8_TYPE), bbList.get(0).array());

                    MRow pRow = ((VMColumnFamilySortedMap) persistentVal).get(pclKey);
                    if (pRow != null)
                    {
    ...
    {
        Iterator<Map.Entry<VMClusteringKey, VMRow>> pcfMapIterator = ((VMColumnFamilySortedMap) persistentVal).getSortedMap().entrySet().iterator();
                                                 ...
                                                 EncodingStats.NO_STATS)
        {
            protected Unfiltered computeNext()
            {
                while (pcfMapIterator.hasNext())
                {
                    ...
                    Map.Entry<VMClusteringKey, VMRow> persistentClusteringKeyMap = pcfMapIterator.next();
                    ...
                    if (filter.getSlices(metadata).hasUpperBound())
                    {
                        ...
                        assert startPersistentClusteringKey != null : "clustering key is null";
                        startSlice = filter.getSlices(metadata).get(0).start().get(0);
                        startPersistentClusteringKey.addClusteringKeyColumn("table_name", MDataTypes.getType(MConfig.PM_UTF8_TYPE), startSlice.array());
                        ...
                        return getUnfilteredRowSingle(pRow, Clustering.make(ByteBuffer.wrap(persistentClusteringKey.getValueBuf(0))), metadata);
                }
                return endOfData();
            }
        };
